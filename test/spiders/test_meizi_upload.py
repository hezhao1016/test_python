...
class mzitu(object):
    ...
    def all_url(self, url):
        ...
        all_a = BeautifulSoup(html.text, 'lxml').find('div', class_='all').find('p', class_='url').find_all('a')
        for a in all_a:
            title = a.get_text()
            print(u'开始保存：%s 套图' % title)  # 加点提示不然太枯燥了
    ...
    def save(self, img_url):  # 这个函数保存图片
        print('正在保存 %s 图片...' % img_url)
        name = img_url[-9:-4]
        img = self.request(img_url)
        f = open(os.path.join(mzitu.current_dir, name) + '.jpg', 'ab')
        f.write(img.content)
        f.close()

    def mkdir(self, path):  # 这个函数创建文件夹
        path = path.strip()
        isExists = os.path.exists(os.path.join(mzitu.base_path, path))
        mzitu.current_dir = os.path.join(mzitu.base_path, path)
        if not isExists:
            print(u'新建 ', path, u' 文件夹')
            os.makedirs(os.path.join(mzitu.base_path, path))
            return True
        else:
