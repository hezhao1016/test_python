...
def main():
    ...
    with open("../../files/raw_result.txt", 'w', encoding='utf-8') as girls:  # 存储路径里的文件夹需要事先创建。
        girls.write(result_bf)
    girls.close()
    print("Store raw data successfully!!!")

    # ****************   Find all <nonscript> nodes and store them   *****************************************
    with open("../../files/noscript_meta.txt", 'w', encoding='utf-8') as noscript_meta:
        with open("../../files/raw_result.txt", 'r', encoding='utf-8') as girls:
            a = girls.read()
            str1 = str(a)

            r = re.findall(
                r'<img class="origin_image zh-lightbox-thumb" data-original="(https://.+\.jpg)" data-rawheight="[0-9]+" data-rawwidth="[0-9]+" src="(https://.+\.jpg)"',
                str1)
            noscript_inner_all = ""
            for url in r:
    ...
    with open("../../files/img_meta.txt", 'w') as img_meta:
        with open("../../files/noscript_meta.txt", 'r', encoding='utf-8') as noscript_meta:
            count = 0
            for eachline in noscript_meta:
                if eachline is not None:
                    ...
                    img_meta.write(line)
                    urllib.request.urlretrieve(img_url, "../../files/images/" + str(count) + ".jpg")  # 一个一个下载图片
                    count += 1
